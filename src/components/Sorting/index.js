            ...
            <div>
                <div className={'hidden lg:flex'}>
                    <div className={'flex items-center space-x-1 text-md lg:text-lg'}>
                        <div className={'text-grey space-x-1'}>
                            <button><Image src={sortIcon} alt={'sort icon'}/></button>
                            <span>Sort By</span></div>
                        <select name="" id={'sort-by'} className={'appearance-none cursor-pointer'} value={sortBy.type}
                                onChange={(e) => handleChange('type', e.target.value)}>
                            {
                                sortTypes.map(sortType => (<option value={sortType} key={sortType} className={'capitalize'}>{sortType}</option>))
                            }
                        </select>
                        <button className={`${sortBy.asc ? 'rotate-180':''}`} onClick={()=> handleChange('asc', !sortBy.asc)}>
